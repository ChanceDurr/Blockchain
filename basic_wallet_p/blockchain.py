...
def new_transaction():
    ...
    # Check if sender has enough coin
    if blockchain.get_user_balance(sender) < int(amount):
        response = {
            'message': f'{sender} does not have enough currency'
        }
    ...
    response = {
        'message': f'{sender} sent {recipient} {amount} coins. Transaction will post to block {index}.'
    }
...
def start_mining():
    ...
    coins_mined = 0
    while coins_mined < coins:
        last_block = blockchain.last_block
        proof = blockchain.proof_of_work(last_block)

        if blockchain.valid_proof(json.dumps(last_block), proof):
    ...
    response = {
        "message": f'You have mined {coins_mined} coins!'
    }
